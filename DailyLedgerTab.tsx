...
export default function DailyLedgerTab({
  ...
  const handleStartEdit = (tx: Transaction): void => {
    setEditingId(tx.id);
    setEditForm({ ...tx });
  };
                        ...
                        <td className="px-3 py-2.5 font-medium text-slate-600">
                          {isEditing ? (
                            <input
                              type="date"
                              value={editForm?.date || ""}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditForm((prev: Transaction | null) => prev ? { ...prev, date: e.target.value } : null)}
                        ...
                        <td className="px-3 py-2.5">
                          {isEditing ? (
                            <select
                              ...
                              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setEditForm((prev: Transaction | null) => prev ? { ...prev, category: e.target.value } : null)}
                        ...
                        <td className="px-3 py-2.5 font-medium text-slate-700 truncate">
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditForm((prev: Transaction | null) => prev ? { ...prev, description: e.target.value } : null)}
                              className="w-full px-1.5 py-0.5 bg-white border border-slate-300 rounded text-xs"
                            />
                          ) : (
                            tx.description
                        ...
                        <td className="px-3 py-2.5 text-slate-500 font-medium">
                          {isEditing ? (
                            <select
                              ...
                              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setEditForm((prev: Transaction | null) => prev ? { ...prev, payment_method: e.target.value } : null)}
                              className="w-full px-1.5 py-0.5 bg-white border border-slate-300 rounded text-xs"
                            >
                              {PAYMENT_METHODS.map((pm: string) => (
                                <option key={pm} value={pm}>
                          ...
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                ...
                                setEditForm((prev: Transaction | null) => prev ? { ...prev, expense_amount: val, income_amount: val > 0 ? 0 : prev.income_amount } : null);
                              }}
                        ...
                        <td className="px-3 py-2.5 text-right font-medium text-emerald-600 bg-emerald-50/10">
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                const val = parseFloat(e.target.value) || 0;
                                setEditForm((prev: Transaction | null) => prev ? { ...prev, income_amount: val, expense_amount: val > 0 ? 0 : prev.expense_amount } : null);
                              }}
